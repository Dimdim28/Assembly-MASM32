...
.DATA
  NESTER_LIB dd ?
  NESTER_PROCEDURE_ADRESS_POINTER dd ?
  NESTER_LIBRARY_NAME db "8-16-IM-13-Nesterov-Library", 0
  NESTER_PROC_NAME db "NESTER_CALCULATE_LIBRARY_EXPR", 0
...
NESTER_MOV_OFFSET MACRO param
  MOV edx, OFFSET param
  PUSH edx
ENDM

NESTER_LEA MACRO param
  LEA edx, param
	PUSH edx
ENDM

NESTER_PREPARE_TO_WORK MACRO
  NESTER_MOV_OFFSET  NESTER_RESULT
  NESTER_MOV_OFFSET  NESTER_CONST1
  NESTER_MOV_OFFSET  NESTER_CONST2
  NESTER_MOV_OFFSET  NESTER_CONST3
	NESTER_LEA NESTER_ARRAY_D[esi * 8]
	NESTER_LEA NESTER_ARRAY_C[esi * 8]
	NESTER_LEA NESTER_ARRAY_B[esi * 8]
	NESTER_LEA NESTER_ARRAY_A[esi * 8]
ENDM

NESTER_CALCULATE MACRO
NESTER_PREPARE_TO_WORK
...
;all checking were made before calling library procedure to avoid useless operations
	CALL [NESTER_PROCEDURE_ADRESS_POINTER]
...
NESTER_SIXTH_LAB:
  MOV esi, 0
    INVOKE LoadLibrary, addr NESTER_LIBRARY_NAME ; library connection/loading
      MOV NESTER_LIB, eax
  INVOKE GetProcAddress, NESTER_LIB, addr NESTER_PROC_NAME ; getting address of the procedure
  MOV NESTER_PROCEDURE_ADRESS_POINTER, eax
